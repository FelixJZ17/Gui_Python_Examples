...
class FormularioGraficasDesign():
    ...
    #Configuro un ejemplo de grafico
    def grafico1(self, ax):
        ...
        ax.set_title('Grafico 1 - Gráfico de barras')
        ax.set_xlabel('Eje X')
        ax.set_ylabel('Eje Y')
        ax.legend()
    ...
    #Configuro otro segundo ejemplo de grafico
    def grafico2(self, ax):
        x = [1, 2, 3, 4, 5]
        y = [1, 2, 1, 2, 1]

        ax.plot(x, y, label='Grafico 2', color='red')
        ax.set_title('Grafico 2')
        ax.set_xlabel('Eje X', fontsize=12)
        ax.set_ylabel('Eje Y', fontsize=12)
        ax.plot(x, y, label='Grafico 2', color='red', linestyle='--', marker='o')
        ax.annotate('Punto importante', xy=(3,1), xytext=(3.5, 1.5),
                    arrowprops=dict(facecolor='black', shrink=0.05))
        
        ax.set_xlim(0, 6) # Establece límites del eje x
        ax.set_ylim(0, 3) # Establece límites del eje y
